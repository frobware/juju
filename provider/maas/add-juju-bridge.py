...
def etc_hosts_hostname():
    return "127.0.1.1 {} # Added by Juju".format(os.uname()[1])

def add_hostname_to_etc_hosts():
    hostname_entry = etc_hosts_hostname()
    with open("/etc/hosts", 'r') as f:
        for line in f:
            if hostname_entry in line:
                return
    with open("/etc/hosts", 'a') as f:
        f.write(hostname_entry + "\n")

def remove_hostname_from_etc_hosts():
    with open("/etc/hosts", 'r') as f:
        lines = f.readlines()
    hostname_entry = etc_hosts_hostname()
    with open("/etc/hosts", 'w') as f:
        for line in lines:
            if hostname_entry not in line:
                f.write(line)
...
if __name__ == '__main__':
    try:
        ...
        remove_hostname_from_etc_hosts()
