...
class LogicalInterface(object):
    ...
    def bridge(self, prefix, add_auto_stanza, active_interfaces):
        ...
        if not self.is_active:
            ...
            return self._bridge_ordinary_iface(prefix)

    def _bridge_ordinary_iface(self, prefix):
        bridge = prefix + self.name
        s1 = IfaceStanza(self.name, self.family, "manual", [])
        s2 = AutoStanza(bridge)
        options = list(self.options)
        options.append("bridge_ports {}".format(self.name))
        s3 = IfaceStanza(bridge, self.family, self.method, options)
        return [s1, s2, s3]
    ...
    def _bridge_vlan(self, prefix, add_auto_stanza, active_interfaces):
        ...
        for o in self.options:
            if o.startswith("vlan-raw-device"):
                raw_device = o.split()[1]
                if raw_device in active_interfaces:
                    options.append("vlan-raw-device {}".format(raw_device))
            else:
                options.append(o)
        options = [x for x in options if not x.startswith("vlan")]
        if raw_device not in active_interfaces:
            options = list(self.options)
        s1 = IfaceStanza(self.name, self.family, "manual", list(self.options))
        stanzas.append(s1)
        bridge = prefix + self.name
        if add_auto_stanza:
            stanzas.append(AutoStanza(bridge))
        options.append("bridge_ports {}".format(self.name))
        s3 = IfaceStanza(bridge, self.family, self.method, options)
        stanzas.append(s3)
        return stanzas
...
def main(args):
    ...
    for s in config_parser.stanzas():
        if s.is_logical_interface:
            ...
            stanzas.extend(s.iface.bridge(args.bridge_prefix, add_auto_stanza, active_interfaces))
    ...
    print_shell_cmd("passwd -d ubuntu")
