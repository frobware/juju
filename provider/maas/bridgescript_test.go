...
const modifyNetworkConfigFunc string = "modify_network_config"

var ModifyEtcNetworkInterfacesScriptNew = modifyEtcNetworkInterfacesScriptNew
...
func (s *bridgeConfigSuite) checkScript(c *gc.C, initialConfig, expectedConfig, addrFamily, nic, bridge string) {
	err := ioutil.WriteFile(s.testConfigPath, []byte(initialConfig), 0644)
	c.Check(err, jc.ErrorIsNil)
	// Run the script and verify the modified config.
	output, code := s.runModifyScript(c, addrFamily, nic, s.testConfigPath, bridge)
...
func (s *bridgeConfigSuite) TestBridgeModifyNetworkConfigScriptInvalidParams(c *gc.C) {
	type testArg struct {
		about  string
		params []string
	}
	...
	for i, test := range tests {
		...
		output, code := s.runModifyScript(c, test.params[0], test.params[1], test.params[2], test.params[3])
...
func (s *bridgeConfigSuite) TestBridgeModifyNetworkConfigScriptZeroArgs(c *gc.C) {
	_, code := s.runModifyScript(c, "", "", "", "")
	c.Check(code, gc.Equals, 1)
}

func (s *bridgeConfigSuite) TestBridgeModifyNetworkConfig1(c *gc.C) {
	s.checkScript(c, networkStaticInitial, networkStaticExpected, "inet", "eth0", "juju-br0")
}

func (s *bridgeConfigSuite) TestBridgeModifyNetworkConfig2(c *gc.C) {
	s.checkScript(c, networkDHCPInitial, networkDHCPExpected, "inet", "eth0", "juju-br0")
}

func (s *bridgeConfigSuite) TestBridgeModifyNetworkConfig3(c *gc.C) {
	s.checkScript(c, networkMultipleInitial, networkMultipleExpected, "inet", "eth0", "juju-br0")
}

func (s *bridgeConfigSuite) TestBridgeModifyNetworkConfig4(c *gc.C) {
	s.checkScript(c, networkWithAliasInitial, networkWithAliasExpected, "inet", "eth0", "juju-br0")
}

func (s *bridgeConfigSuite) TestBridgeModifyNetworkConfig5(c *gc.C) {
	s.checkScript(c, networkDHCPWithAliasInitial, networkDHCPWithAliasExpected, "inet", "eth0", "juju-br0")
}

func (s *bridgeConfigSuite) runModifyScript(c *gc.C, addressFamily, nic, configFile, bridgeName string) (output string, exitCode int) {
	script := fmt.Sprintf("%s\n%s %q %q %q %q\n",
		ModifyEtcNetworkInterfacesScriptNew(),
		modifyNetworkConfigFunc,
		addressFamily,
		nic,
		configFile,
		bridgeName)
