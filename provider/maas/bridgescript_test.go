...
iface test-br-eth1 inet static
    ...
    pre-up ip link add dev eth1 name test-br-eth1 type bridge || true`
...
iface test-br-bond0 inet dhcp
    ...
    hwaddress 52:54:00:1c:f1:5b
    bridge_ports bond0
    pre-up ip link add dev bond0 name test-br-bond0 type bridge || true
...
iface test-br-eth10 inet static
    ...
    pre-up ip link add dev eth10 name test-br-eth10 type bridge || true
...
iface juju-br-eth4 inet static
    ...
    pre-up ip link add dev eth4 name juju-br-eth4 type bridge || true
...
iface juju-br-eth5 inet dhcp
    ...
    pre-up ip link add dev eth5 name juju-br-eth5 type bridge || true
...
iface juju-br-eth6 inet static
    ...
    mtu 1500
    bridge_ports eth6
    up ip link set eth6 up
    pre-up ip link add dev eth6 name juju-br-eth6 type bridge || true
...
iface juju-br-bond0 inet static
    ...
    hwaddress 52:54:00:6a:4f:fd
    bridge_ports bond0
    pre-up ip link add dev bond0 name juju-br-bond0 type bridge || true
    up ip link set bond0 up
...
iface juju-br-bond1 inet dhcp
    ...
    hwaddress 52:54:00:8e:6e:b0
    bridge_ports bond1
    pre-up ip link add dev bond1 name juju-br-bond1 type bridge || true
    up ip link set bond1 up
...
iface vlan-br-eth0 inet static
    ...
    bridge_ports eth0
    up ip link set eth0 up
    pre-up ip link add dev eth0 name vlan-br-eth0 type bridge || true

iface eth1 inet manual

auto vlan-br-eth1
iface vlan-br-eth1 inet dhcp
    mtu 1500
    bridge_ports eth1
    up ip link set eth1 up
    pre-up ip link add dev eth1 name vlan-br-eth1 type bridge || true

iface eth1.2 inet manual

auto vlan-br-eth1.2
iface vlan-br-eth1.2 inet static
    address 192.168.2.1/24
    vlan-raw-device eth1
    up ip link set eth1 up
    mtu 1500
    vlan_id 2
    bridge_ports eth1.2
    up ip link set eth1.2 up
    pre-up ip link add dev eth1.2 name vlan-br-eth1.2
