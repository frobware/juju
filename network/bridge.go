...
func AddBridge(timeout time.Duration, bridgePrefix, inputFile string, deviceNames []string) error {
	...
	command := fmt.Sprintf("%s %q --activate --bridge-prefix=%q --interfaces-to-bridge=%q %q",
		pythonVersion, tmpfile, bridgePrefix, strings.Join(deviceNames, " "), inputFile)

	logger.Infof("running %q with timeout=%v", command, timeout)

	result, err := runCommandWithTimeout(command, time.Duration(timeout), clock.WallClock)
